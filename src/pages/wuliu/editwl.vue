...
<template>
  <div class="contains" style="padding-bottom:50px;">
        <mt-header :title="'修改'+oldUnit" fixed>
            <router-link to="/wlrecord" slot="left">
...
export default {
    ...
    methods: {
        ...
        searchImg(PicBatch){
            let _this = this;
            let parms = _this.jsonFormat({Batch_Number:PicBatch});
            _this.$http({
                method:"post",
                // url:"/wl",
                data:{
                ...
                if( result.state ){
                    ...
                    result.data.Table.forEach(element => {
                        ...
                        this.$refs.uploader.files.push(item)
                    });
                    _this.tb = result.data.Table;
    ...
    created:function(){
        ...
        this.oldUnit = this.$route.params.YDH
        
        this.PicBatch = this.$route.params.PicBatch != "无图片" ? this.$route.params.PicBatch : this.getGuid();
        // 判断是否有图片，有的话就去请求获取图片
        if(this.$route.params.PicBatch != "无图片"){
            this.searchImg(this.PicBatch)
        }
        // 获取物流公司数据
        this.companyName()

        delete this.$route.params.RJGS
        delete this.$route.params.PicBatch
